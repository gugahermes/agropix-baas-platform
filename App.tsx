    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 p-4">
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-400 font-medium">Plataforma Financeira para o Agronegócio</p>
        </div>
...
const LoginCard = ({ role, icon, description, onClick }: { role: string; icon: React.ReactNode; description: string; onClick: () => void }) => (
  <button 
    onClick={onClick}
    className="bg-white/5 backdrop-blur-sm border border-white/10 p-6 rounded-2xl hover:bg-white/10 hover:border-green-500/50 transition-all text-left group"
  >
